...
function App() {
  ...
  async function handleBudgetConfirm({ budget }) {
  setBusy('budget')
  setError('')

  try {
    remember(
      await mildangApi.challenges.setBudget(challengeId, {
        survey,
        budget,
      }),
    )
  ...
  else if (screen === '6_conditionCheckin') content = <ConditionCheckin checkin={checkin} error={error} isFetching={busy === 'checkin'} isLoading={busy === 'checkin-save'} onComplete={handleCheckin} onBack={() => setScreen('mainBoard')}/>
  else if (screen === '7_completeReport') content = <CompleteReport error={error} isLoading={busy === 'share'} isReportLoading={busy === 'report'} onShare={handleShare} report={report} onBack={() => setScreen('mainBoard')} shareCard={shareCard} />
  ...
  else if (screen === 'record_b') content = <RecordB challenge={dashboard?.challenge ?? currentRecordView?.challenge} checkinDays={dashboard?.checkin?.checkinDays ?? currentRecordView?.checkinDays} doneToday={dashboard?.checkin?.doneToday} mealRecords={getDemoMealsForDate(currentRecordView, selectedRecordDate)} selectedDate={selectedRecordDate} todayDate={currentRecordView?.logicalDate ?? currentRecordView?.initialDate} onBack={() => setScreen('record_a')} />
